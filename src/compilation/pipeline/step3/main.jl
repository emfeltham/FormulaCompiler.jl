...
Execute all mathematical function operations (log, exp, sqrt, etc.) using a three-phase approach 
to handle complex dependencies while maintaining zero allocations.

## Three-Phase Execution:

1. **Intermediate Results**: Evaluate input expressions (e.g., column values, previous results) 
   into scratch space positions. This may involve recursive function calls or simple data access.

2. **Unary Function Application**: Apply mathematical functions (log, exp, sqrt, sin, cos, abs, ^, boolean ops)
   to intermediate values and store results back in scratch space.

3. **Final Assignment**: Copy function results from scratch space to their final output positions.

## Arguments:
- `function_data`: Pre-computed data containing input specs, functions, and output positions
- `scratch`: Pre-allocated scratch space vector for intermediate computations
- `output`: Final output vector to populate (modified in-place)
- `data`: Raw input data (NamedTuple of columns)
- `row_idx`: Row index to evaluate (1-based)

## Performance:
This approach achieves zero-allocation execution by:
- Using pre-allocated scratch space for all intermediate values
- Processing functions in dependency order
- Avoiding temporary array creation during computation

**Current Issue**: Steps 3&4 have allocation problems (~32-864 bytes) due to symbol-based
column access instead of Val{Column} dispatch patterns used in Steps 1&2.
"""
