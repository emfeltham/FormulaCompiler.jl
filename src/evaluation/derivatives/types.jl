...
"""
Zero-allocation AD evaluator that eliminates all closures and uses DiffResult pre-allocation.
Based on ForwardDiff best practices for true zero-allocation automatic differentiation.
"""
mutable struct ZeroAllocADEvaluator{T, Ops, S, O, NTBase, NTMerged, N, ColsT, JR, JC}
    # Core evaluator components
    compiled_base::UnifiedCompiled{T, Ops, S, O}
    base_data::NTBase
    vars::Vector{Symbol}
    
    # Pre-allocated ForwardDiff workspace - key to zero allocation
    x_buffer::Vector{Float64}
    jacobian_result::JR
    jacobian_config::JC
    
    # Dual-typed evaluation infrastructure
    dual_cache::DualCacheDict
    data_over::NTMerged
    overrides::Vector{FDOverrideVector}
...
"""
Static evaluation function stored as a function pointer - completely eliminates closures
"""
struct StaticEvaluationFunction{ZE}
    evaluator::ZE
end

function (sef::StaticEvaluationFunction)(x::Vector{T}) where T
    de = sef.evaluator
    
    # Use cached dual structures - no rebuilding
    if T === Float64
        compiled_T = de.compiled_base
        # Use pre-allocated buffer - NO allocation!
        row_vec = resize!(de.fd_yplus, length(de.compiled_base))
        ov_vec = de.overrides
        data_over = de.data_over
    else
        # Use persistent dual cache
        cache = get_or_create_cache!(de.dual_cache, T, de, de.row)
        compiled_T = cache.compiled_dual
        row_vec = cache.rowvec_dual
        ov_vec = cache.overrides_dual
        data_over = cache.data_over_dual
    ...
    # Direct evaluation - no closure allocation
    compiled_T(row_vec, data_over, de.row)
    return row_vec
end
