...
###############################################################################
# UNIFIED KRONECKER PATTERN COMPUTATION
###############################################################################

"""
    compute_kronecker_pattern(component_widths::Vector{Int}) -> Vector{NTuple{N,Int}} where N

Unified Kronecker pattern computation with compile-time optimization.
Always returns NTuple for type stability and automatic method dispatch.

# Performance optimizations:
- Single memory allocation
- Type-stable NTuple return enables method dispatch
- Efficient Cartesian product iteration
- Input validation with clear error messages

# Examples
```julia
julia> compute_kronecker_pattern([2, 3])
6-element Vector{NTuple{2,Int}}:
 (1, 1)
 (1, 2)
 (1, 3)
 (2, 1)
...
function compute_kronecker_pattern(component_widths::Vector{Int})
    ...
    isempty(component_widths) && return NTuple{0,Int}[]
    
    # Validate input - all positive integers
    ...
    if N == 1
        w = component_widths[1]
        return [ntuple(i -> i, 1) for i in 1:w]
    end
    
    # Pre-compute total terms and allocate once
    total_terms = prod(component_widths)
    
    # Type-stable pattern allocation with compile-time known N
    pattern = Vector{NTuple{N,Int}}(undef, total_terms)
    
    # Generate ranges tuple for type stability
    ranges = ntuple(i -> 1:component_widths[i], N)
    
    # Fill pattern with minimal allocations
    idx = 1
    @inbounds for combo in Iterators.product(ranges...)
