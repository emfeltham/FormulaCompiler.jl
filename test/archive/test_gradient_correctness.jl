# test_gradient_correctness.jl - Mathematical Correctness Tests for Phase 1 Parameter Gradients
...
@testset "Phase 1 Parameter Gradient Mathematical Correctness" begin
    ...
    marginal_effects_mu!(g_ad, Gβ_ad, de_ad, β, IdentityLink(), test_row)
    marginal_effects_mu!(g_fd, Gβ_fd, de_fd, β, IdentityLink(), test_row)
    ...
    marginal_effects_eta!(g_eta_ad, Gβ_eta_ad, de_ad, β, test_row)
    marginal_effects_eta!(g_eta_fd, Gβ_eta_fd, de_fd, β, test_row)
    ...
    for (link_name, link) in [("Log", LogLink()), ("Logit", LogitLink()), ("Probit", ProbitLink())]
        ...
        marginal_effects_mu!(g_ad, Gβ_ad, de_ad, β, link, test_row)
        marginal_effects_mu!(g_fd, Gβ_fd, de_fd, β, link, test_row)
        ...
        for i in 1:length(β)
            ...
            marginal_effects_mu!(g_plus, de_fd, β_plus, link, test_row)
            marginal_effects_mu!(g_minus, de_fd, β_minus, link, test_row)
