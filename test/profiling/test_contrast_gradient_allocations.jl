...
@testset "Contrast Gradient Allocation Tests" begin
    ...
    @testset "Simple Model Allocations" begin
        ...
        @testset "Warmup and Baseline" begin
            ...
            for i in 1:10
                ...
                contrast_gradient!(∇β, evaluator, row, var, from, to, β)
                delta_method_se(evaluator, row, var, from, to, β, vcov_matrix)
            end
        ...
        @testset "Core Function Allocations" begin
            ...
            allocs_contrast = @allocated contrast_modelrow!(contrast_buf, evaluator, row, var, from, to)
            @debug "contrast_modelrow! allocations: ", allocs_contrast, " bytes"

            # Our functions should have similar or lower allocations
            allocs_grad_linear = @allocated contrast_gradient!(∇β, evaluator, row, var, from, to, β)
            @debug "contrast_gradient! (linear) allocations: ", allocs_grad_linear, " bytes"

            # Test delta method
            allocs_delta = @allocated delta_method_se(evaluator, row, var, from, to, β, vcov_matrix)
        ...
        @testset "Link Function Allocations" begin
            ...
            for (name, link) in links_to_test
                allocs_response = @allocated contrast_gradient!(∇β, evaluator, row, var, from, to, β, link)
                allocs_delta_link = @allocated delta_method_se(evaluator, row, var, from, to, β, vcov_matrix, link)
        ...
        @testset "Performance Benchmarks" begin
            ...
            @btime contrast_modelrow!($contrast_buf, $evaluator, $row, $var, $from, $to)

            @debug "Linear scale gradient:"
            @btime contrast_gradient!($∇β, $evaluator, $row, $var, $from, $to, $β)

            @debug "Logit gradient:"
            link = GLM.LogitLink()
            @btime contrast_gradient!($∇β, $evaluator, $row, $var, $from, $to, $β, $link)

            @debug "Delta method SE:"
            @btime delta_method_se($evaluator, $row, $var, $from, $to, $β, $vcov_matrix)
    ...
    @testset "Complex Model Allocations" begin
        ...
        # Warmup
        contrast_gradient!(∇β, evaluator, 1, :treatment, "Control", "Drug_A", β)
        contrast_gradient!(∇β, evaluator, 1, :binary_var, 0, 1, β)

        @testset "Categorical vs Binary Variable Allocations" begin
            # Categorical variable
            allocs_cat = @allocated contrast_gradient!(∇β, evaluator, 1, :treatment, "Control", "Drug_A", β)
            @debug "Categorical variable gradient allocations: ", allocs_cat, " bytes"

            # Binary variable (should potentially be more efficient)
            allocs_bin = @allocated contrast_gradient!(∇β, evaluator, 1, :binary_var, 0, 1, β)
