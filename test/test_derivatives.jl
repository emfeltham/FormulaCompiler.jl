...
@testset "Derivative correctness" begin
    ...
    @testset "ForwardDiff and FD fallback" begin
        ...
        derivative_modelrow!(J_ad, de_ad, 2)

        # Compare AD and FD on same row (3)
        derivative_modelrow!(J_ad, de_ad, 3)
        derivative_modelrow!(J_fd, de_fd, 3)
        ...
        # println("Max |AD - Manual|: ", maximum(abs.(J_ad .- J_manual)))
        # println("Max |FD - Manual|: ", maximum(abs.(J_fd .- J_manual)))
        # println("J_ad (AD) first column: ", J_ad[:, 1])
        ...
        contrast_evaluator = contrastevaluator(compiled, data, [:group3])
        contrast_modelrow!(Δ, contrast_evaluator, row, :group3, "A", "B")

        # Validate against manual data substitution
        # IMPORTANT: Must use copy() to preserve CategoricalArray structure and levels.
        # Using fill("A", n) creates Vector{String} which loses level metadata,
        # breaking contrast coding and causing silent errors in interaction terms.
        # See CLAUDE.md and docs/counterfactual_analysis.md for detailed explanation.
        ...
        derivative_modelrow!(Jrow, de_ad, row)
        ...
        J_ad_check = Matrix{Float64}(undef, length(compiled), length(vars))
        J_fd_check = Matrix{Float64}(undef, length(compiled), length(vars))
        derivative_modelrow!(J_ad_check, de_ad, row)
        derivative_modelrow!(J_fd_check, de_fd, row)
        # println("\n=== Jacobian Comparison (Row $row) ===")
        # println("Group value at row $row: ", data.group3[row])
        # println("Max |J_ad - J_fd|: ", maximum(abs.(J_ad_check .- J_fd_check)))
        # println("J_ad first column: ", J_ad_check[:, 1])
        # println("J_fd first column: ", J_fd_check[:, 1])
        # println("Difference first column: ", J_fd_check[:, 1] .- J_ad_check[:, 1])
    ...
    @testset "Single-column FD and parameter gradients" begin
        ...
        @testset "fd_jacobian_column!" begin
            ...
            for (i, var) in enumerate(vars)
                ...
                fd_jacobian_column!(Jk, de_fd, test_row, i)
            ...
            derivative_modelrow!(J_fd_standalone, de_fd, test_row)
        ...
        # NOTE: Tests for me_eta_grad_beta! and me_mu_grad_beta! removed
        # These functions don't exist - parameter gradients are now obtained
        # via the Gβ buffer in marginal_effects_eta!/mu! calls
    end
    ...
    @testset "GLM(Logit) and MixedModels" begin
        ...
        de_glm = derivativeevaluator_ad(compiled_glm, data, vars)
        J = Matrix{Float64}(undef, length(compiled_glm), length(vars))
        derivative_modelrow!(J, de_glm, r)  # warm path
        # FD compare
        de_glm_fd = derivativeevaluator_fd(compiled_glm, data, vars)
        J_fd = similar(J)
        derivative_modelrow!(J_fd, de_glm_fd, r)
        ...
        de_mm = derivativeevaluator_ad(compiled_mm, data, vars)
        Jmm = Matrix{Float64}(undef, length(compiled_mm), length(vars))
        derivative_modelrow!(Jmm, de_mm, 2)
        de_mm_fd = derivativeevaluator_fd(compiled_mm, data, vars)
        Jmm_fd = similar(Jmm)
        derivative_modelrow!(Jmm_fd, de_mm_fd, 3)
    ...
    @testset "FD backend robustness and edge cases" begin
        ...
        @testset "Different step sizes" begin
            ...
            # Auto step (default)
            derivative_modelrow!(J_auto, de, test_row)
            ...
            derivative_modelrow!(J_small, de, test_row)
            derivative_modelrow!(J_large, de, test_row)
        ...
        @testset "All categorical combinations" begin
            ...
            for test_row in rows_to_test
                ...
                J_fd = Matrix{Float64}(undef, length(compiled), length(vars))
                derivative_modelrow!(J_fd, de, test_row)
        ...
        @testset "Many variables FD scaling" begin
            ...
            de_many = derivativeevaluator_fd(compiled_many, data_many, vars_many)
            
            # Test FD with many variables
            test_row = 10
            J_fd_many = Matrix{Float64}(undef, length(compiled_many), length(vars_many))
            derivative_modelrow!(J_fd_many, de_many, test_row)
...
@testset "Variance Computation Primitives" begin
    ...
    β_lm = coef(model_lm)
    Σ_lm = vcov(model_lm)
    de_lm = derivativeevaluator_ad(compiled_lm, data, vars)
    ...
    de_glm = derivativeevaluator_ad(compiled_glm, data_logit, vars)
    ...
    # NOTE: Tests for accumulate_ame_gradient! removed - function doesn't exist
    # AME gradient accumulation should be done in Margins.jl by looping over rows
    # and manually accumulating parameter gradients from marginal_effects_eta!/mu!
    ...
    @testset "Integer Continuous Variables Derivatives" begin
        ...
        @testset "Integer variable derivatives - basic" begin
            ...
            vars = [:int_x]
            de = derivativeevaluator_ad(compiled, data_int, vars)
            ...
            # Test FD derivatives
            de_fd = derivativeevaluator_fd(compiled, data_int, vars)
            J_fd = Matrix{Float64}(undef, length(compiled), length(vars))
