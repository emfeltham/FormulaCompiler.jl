...
@testset "FD log profile regression" begin
    ...
    de_fd = derivativeevaluator(:fd, compiled, data, [:x])
    de_ad = derivativeevaluator(:ad, compiled, data, [:x])
    ...
    @test_nowarn fd_jacobian_column!(Jk, de_fd, row, x_idx)
    ...
    @test_nowarn derivative_modelrow!(J, de_fd, row)
    @test J[1, 1] ≈ 0.0 atol=1e-12
    @test J[2, 1] ≈ 1.0 / xval rtol=1e-8

    # 3) AD marginal effects on η should match β1/x
    β = coef(model)
